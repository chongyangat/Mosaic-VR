...
本目录整理当前论文 Figure 7 的图片、实验记录、处理与绘图代码，以及第二轮复核资料。原文件按原样复制，未重新分析数据。
...
- `source_package/05_Data/reproduced_output/Figure_source_latency_curves.csv`：绘图曲线数据；`scene=supermarket`是本图数据。`participant=aggregate`表示跨参与者中位数及四分位距，其他行表示各参与者曲线。前三条链路按1秒分箱，状态同步RTT按5秒分箱。
...
原绘图脚本读取全部20个CSV、15个最小GC日志，并用两个场景共同确定相同纵轴范围。因此Figure 7、8、9各自保留这35个输入，便于不修改代码独立复现；街道数据也是本图原始生成流程的依赖。

超市场景有10个CSV、7个直接GC日志；P01、P02、P03超市运行没有直接日志，现行脚本使用跨链路候选识别。共同保存的街道输入中，P01、P02也没有直接日志。没有用其他运行日志填补这些缺口。

## 复现入口

在本Figure_07目录、具有Python环境的终端运行：
...
第一份脚本重新生成整组基线图及曲线/统计表；第二份脚本补充配对场景检验。结果写到`source_package/05_Data/reproduced_output/`，本图输出名为`Figure_supermarket_latency_stability`，含PNG/SVG/PDF/TIFF。它不会自动更新`figure/`中的归档PNG。请保持现有目录层级，不要从结果目录中的历史脚本副本运行。Arial字体会影响与既有图的外观一致性。
...
归档载荷：82 个文件，514.1 MiB。复制和提取文件均有 SHA-256 记录。
