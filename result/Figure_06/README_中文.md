...
- `external_source_assets/figures/fig_experiment_control/output/MOSAIC_VR_dual_end_state_interaction.drawio`
- `source_package/03_Figure_Sources_Internal/Figure_06/Figure_06_bidirectional_authoritative_state_propagation.drawio`

## 本图数据与使用说明

本图由系统架构、流程、照片或软件截图构成，没有用于生成本图的定量实验 CSV。当前 draw.io 是可编辑组合源，照片和截图原字节已拆出到 drawio_subfigures。历史编排脚本及旧素材保留其来源说明；后续人工调整的版式不保证由旧脚本一键重现。


归档载荷：28 个文件，239.5 MiB。复制和提取文件均有 SHA-256 记录。
