...
namespace GameMain
{
    ...
    public class UILoadTip : UIBase
    {
        ...
        private void OnEnable()
        {
            ...
            _btn_package.onClick.AddListener(OnGamePackage);
        }

        private void OnDisable()
        {
            _btn_update.onClick.RemoveListener(OnGameUpdate);
            _btn_ignore.onClick.RemoveListener(OnGameIgnore);
            _btn_package.onClick.RemoveListener(OnGamePackage);
        }
        ...
        /// <param name="cancle">取消按钮文字</param>
        /// <param name="package">更新按钮文字</param>
        public static void ShowMessageBox(string desc, MessageShowType showtype = MessageShowType.OneButton,
            ...
            string ok = null, string cancle = null, string package = null)
        {
            ...
            if (ui == null) return;
            // 设置按钮文字
            if (ok != null) ui._btn_update.GetComponentInChildren<Text>().text = ok;
            if (cancle != null) ui._btn_ignore.GetComponentInChildren<Text>().text = cancle;
            if (package != null) ui._btn_package.GetComponentInChildren<Text>().text = package;
            // 
            ui.OnOk = onOk;
            ui.OnCancle = onCancel;
            ui.OnPackage = onPackage;
            // 
