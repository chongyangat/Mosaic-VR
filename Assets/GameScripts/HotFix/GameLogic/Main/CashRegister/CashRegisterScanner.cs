...
// Author：ASSJ
// CreateTime：2024/8/27 11:17:16
// Version: v1.0
// Description：自助收银机扫描器
// ===================================================
...
namespace GameLogic
{
    ...
    /// 自助收银机扫描器
    /// </summary>
    public class CashRegisterScanner : MonoBehaviour
    {
        [Tooltip("货物层")]
        [SerializeField]
        ...
        private bool ScanProducts(Product product)
        {
            ...
            {
                // 输出日志
                Debug.LogWarning("收银机扫描器检测到物体，但该物体没有Good组件");
                return false;
            }
            // 输出日志
            Debug.Log($"收银机扫描器扫描到该商品((id:{product.Id}){product.name})");
            // 
            ...
            {
                // 输出日志
                Debug.LogWarning($"收银机扫描器添加该商品((id:{product.Id}){product.name})失败");
                return false;
            }
            // 添加成功
            // 输出日志
            Debug.Log($"收银机扫描器成功添加该商品((id:{product.Id}){product.name})");
            return true;
        }
        ...
        private void OnTriggerEnter(Collider other)
        {
            ...
            {
                // 输出日志
                Debug.Log($"收银机扫描器检测到物体{other.name}进入");
                //
        ...
        private void OnTriggerExit(Collider other)
        {
            ...
            {
                // 输出日志
                Debug.Log($"收银机扫描器检测到物体{other.name}退出");
            }
        }
